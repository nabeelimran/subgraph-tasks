...
import { ApprovalEntity, BurnEntity, TransferEntity, User, TokenStat } from "../generated/schema"
...
export function handleTransfer(event: Transfer): void {
  ...
  let receiver = User.load(event.params._to.toHex());
  let sender = User.load(event.params._from.toHex());
  ...
  if (!receiver) {
    receiver = new User(event.params._to.toHex());
  ...
  if (!sender) {
    sender = new User(event.params._from.toHex());
    tokenStat._holders = tokenStat._holders.plus(BigInt.fromI64(1));
  }
