...
import { StakeEntity, UnstakeEntity, User, Transaction, Contract } from "../generated/schema"
...
export function handleStakeCompleted(event: StakeCompleted): void {
  ...
  let user = User.load(event.params.staker.toHex())
  if (!user) {
    user = new User(event.params.staker.toHex())
  }
